...
🤖 ᴍᴀᴅᴇ ʙʏ https://t.me/ITSGOLU_OFFICIAL"""
...
• ᴘʀᴇᴍɪᴜᴍ ᴋᴇ ʟɪʏᴇ @ITsGOLU_OWNER_BOT ᴘᴀʀ ᴍᴇꜱꜱᴀɢᴇ ᴋᴀʀᴇ"""
...
async def get_welcome_buttons(user_id: int = None):
    ...
    buttons.append([
        InlineKeyboardButton("📢 ᴜᴘᴅᴀᴛᴇs", url="https://t.me/ITSGOLU_OFFICIAL")
    ])
...
💰 **ᴘʱɪᴄᴇ:** ₹500 ᴏɴʟʏ!"""
    ...
    buttons = [
        [InlineKeyboardButton(
            "🛍️ ʙᴜʏ ɴᴏᴡ",
            url=f"https://t.me/ITsGOLU_OWNER_BOT"
        )],
...
@app.on_callback_query(filters.regex(r"^earn_credits_(\d+)$"))
        ...
        if mode == 'premium':
            ...
            if not user.get('is_premium', False):
                await callback_query.answer(
                    ...
                    "Contact @ITsGOLU_OWNER_BOT to get premium access.",
                    show_alert=True
                )
                return
        else:  # demo mode
...
async def handle_account_callback(client, callback_query):
    ...
    try:
        ...
        account_msg = (
            ...
            f"📞 ᴄᴏɴᴛᴀᴄᴛ @ITsGOLU_OWNER_BOT ғᴏʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss"
        )
...
@app.on_message(filters.command(["add", "remove"]) & filters.private)
async def handle_premium_command(client, message):
    ...
    try:
        ...
        user_id = int(args[0])
        days = int(args[1])
        is_add = message.command[0].lower() == "add"
...
@app.on_message(filters.text & filters.private & ~filters.command(["add", "remove", "start", "help", "clear", "getdb", "decx"]))
...
@app.on_callback_query(filters.regex(r"^series_(\d+)$"))
        ...
        if not await is_premium(user_id):
            ...
            if credits <= 0:
                await message.reply(
                    ...
                    "📞 ᴄᴏɴᴛᴀᴄᴛ @ITsGOLU_OWNER_BOT ᴛᴏ ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss."
                )
                return
